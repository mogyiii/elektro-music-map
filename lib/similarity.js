...
const CHANGE_WORDS = [
  [0.15, "végig ugyanaz"],
  [0.35, "keveset változik"],
  [0.55, "mérsékelten változik"],
  [0.78, "sokat változik"],
  [1.01, "szinte semmi nem ismétlődik"],
];
  ...
  for (const [key, axis] of Object.entries(axesConfig)) {
    if (axis.type === "numeric") {
      ...
      if (key === "tempo") {
        ...
        rows.push({ label: axis.label, value: FUNCTION_WORDS.find(([t]) => mid < t)[1] });
      } else if (key === "change") {
        rows.push({ label: axis.label, value: CHANGE_WORDS.find(([t]) => mid < t)[1] });
      } else {
    ...
    rows.push({
      ...
      value: same
        ? namesA.join(" + ")
        : `${namesA.join("/")} és ${namesB.join("/")} között`,
    });
  }
